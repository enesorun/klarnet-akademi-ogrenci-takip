                    ...
                    gruplar.map((grup) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getDurumColor(grup.durum)}`}
                          >
                            {getDurumLabel(grup.durum)}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            ...
                            <Button
                              onClick={() => handleOpenGrupModal(grup)}
                              variant="outline"
                              size="sm"
                              className="dark:border-gray-600 dark:text-white dark:hover:bg-gray-700"
                            >
                            ...
                            <Button
                              onClick={() => setDeleteGrupId(grup.id)}
                              variant="outline"
                              size="sm"
      ...
      <Dialog open={isGrupModalOpen} onOpenChange={setIsGrupModalOpen}>
        <DialogContent className="dark:bg-gray-800">
          ...
          <form onSubmit={handleCreateGrup} className="space-y-4">
            ...
            {grupOzelAlanlar.length > 0 && (
              <div className="border-t border-gray-200 dark:border-gray-700 pt-4 mt-4">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                  Özel Bilgiler
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {grupOzelAlanlar.map((alan) => (
                    <div key={alan.id}>
                      <Label htmlFor={`grup_ozel_${alan.id}`} className="dark:text-gray-300">
                        {alan.alan_adi}
                      </Label>
                      {alan.alan_tipi === "text" && (
                        <Input
                          id={`grup_ozel_${alan.id}`}
                          value={grupOzelAlanlarData[alan.id] || ""}
                          onChange={(e) =>
                            setGrupOzelAlanlarData({ ...grupOzelAlanlarData, [alan.id]: e.target.value })
                          }
                          className="dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                        />
                      )}
